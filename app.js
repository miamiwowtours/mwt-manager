...
// Channel-specific retail pricing overrides.
// If a channel + product appears here, these prices replace the product's
// default adult/child price. Applies to NEW bookings logged from deploy date
// onward; previously-saved tours keep their stored figures.
var CHANNEL_PRICING = {
  GYG: { 'Miami Wow Tour': {adultPrice:67, childPrice:52} }
};
function priceFor(product, channel) {
  var base = TOURS[product] || TOURS['Miami Wow Tour'];
  var o = CHANNEL_PRICING[channel] && CHANNEL_PRICING[channel][product];
  return {
    adultPrice: o ? o.adultPrice : base.adultPrice,
    childPrice: o ? o.childPrice : base.childPrice,
    privatePrice: base.privatePrice
  };
}
...
function calcTourData(adults, children, infants, guide, channel, type, otherExp, product, adultOverride, childOverride) {
  var tour = priceFor(product, channel);
  var ap = (adultOverride != null && !isNaN(adultOverride) && adultOverride >= 0) ? adultOverride : tour.adultPrice;
  var cp = (childOverride != null && !isNaN(childOverride) && childOverride >= 0) ? childOverride : tour.childPrice;
  var gross = type === 'private' ? tour.privatePrice : (adults * ap + children * cp);
...
function calcTour() {
  ...
  var aOv=parseFloat(document.getElementById('f-adult-override').value);
  var cOv=parseFloat(document.getElementById('f-child-override').value);
  document.getElementById('f-adult-override').placeholder=tour.adultPrice;
  document.getElementById('f-child-override').placeholder=tour.childPrice;
  var d=calcTourData(adults,children,infants,guide,channel,type,exp,product,aOv,cOv);
...
function clearForm() {
  ...
  document.getElementById('f-adult-override').value='';
  document.getElementById('f-child-override').value='';
...
function openEditModal(id){var t=tours.find(function(t){return t.id===id;});if(!t)return;editTourId=id;var product=t.product||'Miami Wow Tour';document.getElementById('em-product').value=product;document.getElementById('em-date').value=t.date;document.getElementById('em-type').value=t.type||'regular';document.getElementById('em-guide').value=t.guide;document.getElementById('em-adults').value=t.adults||0;document.getElementById('em-children').value=t.children||0;document.getElementById('em-infants').value=t.infants||0;document.getElementById('em-channel').value=t.channel;document.getElementById('em-expenses').value=t.otherExp||0;document.getElementById('em-adult-override').value='';document.getElementById('em-child-override').value='';document.getElementById('em-notes').value=t.notes||'';var pax=(t.adults||0)+(t.children||0)+(t.infants||0)||getPax(t);var autoGP=t.type==='private'?80:(t.guide==='Noel'?(pax>3?100:75):(pax>3?75:60));document.getElementById('em-guidepay').value=(t.guidePay!==undefined&&Math.abs(t.guidePay-autoGP)>0.01)?t.guidePay.toFixed(2):'';var tour=TOURS[product]||TOURS['Miami Wow Tour'];document.getElementById('em-adults-label').textContent='Adults ($'+tour.adultPrice+')';document.getElementById('em-children-label').textContent='Children ($'+tour.childPrice+')';document.getElementById('edit-modal-bg').style.display='flex';setTimeout(function(){previewEdit();},50);}
function previewEdit(){var product=document.getElementById('em-product').value;var adults=parseInt(document.getElementById('em-adults').value,10)||0;var children=parseInt(document.getElementById('em-children').value,10)||0;var infants=parseInt(document.getElementById('em-infants').value,10)||0;var guide=document.getElementById('em-guide').value;var channel=document.getElementById('em-channel').value;var type=document.getElementById('em-type').value;var exp=parseFloat(document.getElementById('em-expenses').value)||0;var aOv=parseFloat(document.getElementById('em-adult-override').value);var cOv=parseFloat(document.getElementById('em-child-override').value);var tour=priceFor(product, channel);document.getElementById('em-adults-label').textContent='Adults ($'+tour.adultPrice+')';document.getElementById('em-children-label').textContent='Children ($'+tour.childPrice+')';document.getElementById('em-adult-override').placeholder=tour.adultPrice;document.getElementById('em-child-override').placeholder=tour.childPrice;var d=calcTourData(adults,children,infants,guide,channel,type,exp,product,aOv,cOv);document.getElementById('em-gross').textContent='$'+d.gross.toFixed(2);document.getElementById('em-comm').textContent='-$'+d.comm.toFixed(2)+' ('+Math.round(CHANNELS[channel].rate*100)+'%)';var fhflatRow=document.getElementById('em-fhflat-row');if(fhflatRow)fhflatRow.style.display=channel==='Fareharbor'?'flex':'none';var fhprocRow=document.getElementById('em-fhproc-row');if(fhprocRow)fhprocRow.style.display='flex';document.getElementById('em-fhproc').textContent='-$'+d.fhProc.toFixed(2);var gpOverride=parseFloat(document.getElementById('em-guidepay').value);var gpDisplay=(!isNaN(gpOverride)&&gpOverride>=0)?gpOverride:d.guidePay;document.getElementById('em-guide-pay').textContent='-$'+gpDisplay.toFixed(2)+((!isNaN(gpOverride)&&gpOverride>=0)?' (override)':' (auto)');var drinksRow=document.getElementById('em-drinks-row');if(drinksRow)drinksRow.style.display=product==='Miami Wow Tour'?'flex':'none';document.getElementById('em-drinks').textContent='-$'+d.drinks.toFixed(2);document.getElementById('em-exp').textContent=exp>0?'-$'+exp.toFixed(2):'$0.00';var pp=Math.round((d.gross-d.comm-(d.fhFlat||0)-(d.fhProc||0)-gpDisplay-(d.drinks||0)-exp)*100)/100;var pel=document.getElementById('em-profit');pel.textContent=(pp>=0?'$':'-$')+Math.abs(pp).toFixed(2);pel.style.color=pp>=0?'var(--teal)':'var(--red)';}
function saveEdit(){if(!editTourId)return;var t=tours.find(function(t){return t.id===editTourId;});if(!t)return;var product=document.getElementById('em-product').value;var adults=parseInt(document.getElementById('em-adults').value,10)||0;var children=parseInt(document.getElementById('em-children').value,10)||0;var infants=parseInt(document.getElementById('em-infants').value,10)||0;var guide=document.getElementById('em-guide').value;var channel=document.getElementById('em-channel').value;var type=document.getElementById('em-type').value;var exp=parseFloat(document.getElementById('em-expenses').value)||0;var notes=document.getElementById('em-notes').value;var date=document.getElementById('em-date').value;if(!date){toast('Please enter a date.',true);return;}var aOv=parseFloat(document.getElementById('em-adult-override').value);var cOv=parseFloat(document.getElementById('em-child-override').value);var d=calcTourData(adults,children,infants,guide,channel,type,exp,product,aOv,cOv);var gpOverride=parseFloat(document.getElementById('em-guidepay').value);var finalGuidePay=(!isNaN(gpOverride)&&gpOverride>=0)?Math.round(gpOverride*100)/100:d.guidePay;var finalProfit=Math.round((d.gross-d.comm-(d.fhFlat||0)-(d.fhProc||0)-finalGuidePay-(d.drinks||0)-exp)*100)/100;t.product=product;t.date=date;t.type=type;t.guide=guide;t.adults=adults;t.children=children;t.infants=infants;t.pax=adults+children+infants;t.channel=channel;t.otherExp=exp;t.notes=notes;t.gross=d.gross;t.comm=d.comm;t.fhFlat=d.fhFlat;t.fhProc=d.fhProc;t.vendorNet=Math.round((d.gross-d.comm-(d.fhFlat||0))*100)/100;t.guidePay=finalGuidePay;t.drinks=d.drinks;t.profit=finalProfit;saveTours();saveTour(t);closeEditModal();renderTours();renderDashboard();renderGuides();toast('Tour updated!');}
