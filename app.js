...
function renderDashboard() {
  ...
  if(last5.length===0){document.getElementById('recent-table').innerHTML='<div class="empty">No tours yet.</div>';return;}
...
function renderPayReport() {
  ...
  var sg=document.getElementById('pr-guide').value;
  var data=tours.filter(function(t){return(m==='all'||fmtM(t.date)===parseInt(m))&&(sg==='all'||t.guide===sg);}).sort(function(a,b){return new Date(b.date)-new Date(a.date);});
  var totPay=data.reduce(function(s,t){return s+t.guidePay;},0);
  var totDrinks=data.reduce(function(s,t){return s+(t.drinks||0);},0);
  var totOther=data.reduce(function(s,t){return s+(t.otherExp||0);},0);
  document.getElementById('pr-summary').innerHTML='<div class="metric"><div class="metric-label">Tours worked</div><div class="metric-value">'+data.length+'</div></div><div class="metric"><div class="metric-label">Total tour pay</div><div class="metric-value green">$'+totPay.toFixed(2)+'</div></div><div class="metric"><div class="metric-label">Total reimbursements</div><div class="metric-value amber">$'+(totDrinks+totOther).toFixed(2)+'</div></div>';
  if(data.length===0){document.getElementById('pr-content').innerHTML='<div class="card"><div class="empty">No tours found.</div></div>';return;}
  var byGuide={};
  data.forEach(function(t){if(!byGuide[t.guide])byGuide[t.guide]=[];byGuide[t.guide].push(t);});
  var html='';
  Object.keys(byGuide).sort().forEach(function(guide){
    var gt=byGuide[guide];
    var gPay=gt.reduce(function(s,t){return s+t.guidePay;},0);
    var gDrinks=gt.reduce(function(s,t){return s+(t.drinks||0);},0);
    var gOther=gt.reduce(function(s,t){return s+(t.otherExp||0);},0);
    var gReimb=gDrinks+gOther,gTotal=gPay+gReimb;
    var period=m==='all'?'Full year 2026':FULLMONTHS[parseInt(m)];
    html+='<div class="card" style="margin-bottom:1.25rem;"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1rem;flex-wrap:wrap;gap:12px;"><div><div style="font-size:18px;font-weight:600;">'+guide+'</div><div style="font-size:12px;color:var(--text3);">'+gt.length+' tour'+(gt.length!==1?'s':'')+' &bull; '+period+'</div></div><div style="display:flex;gap:20px;text-align:right;flex-wrap:wrap;"><div><div style="font-size:11px;color:var(--text3);text-transform:uppercase;font-weight:600;">Tour pay</div><div style="font-size:18px;font-weight:600;font-family:DM Mono,monospace;color:var(--teal);">$'+gPay.toFixed(2)+'</div></div><div><div style="font-size:11px;color:var(--text3);text-transform:uppercase;font-weight:600;">Drinks</div><div style="font-size:18px;font-weight:600;font-family:DM Mono,monospace;color:var(--amber);">$'+gDrinks.toFixed(2)+'</div></div><div><div style="font-size:11px;color:var(--text3);text-transform:uppercase;font-weight:600;">Other reimb.</div><div style="font-size:18px;font-weight:600;font-family:DM Mono,monospace;color:var(--amber);">$'+gOther.toFixed(2)+'</div></div><div><div style="font-size:11px;color:var(--text3);text-transform:uppercase;font-weight:600;">Total owed</div><div style="font-size:18px;font-weight:600;font-family:DM Mono,monospace;">$'+gTotal.toFixed(2)+'</div></div></div></div>';
    html+='<div class="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Channel</th><th>Guests</th><th>Tour pay</th><th>Drinks</th><th>Other</th><th>Total</th><th>Notes</th></tr></thead><tbody>';
    gt.forEach(function(t){var dr=t.drinks||0,oe=t.otherExp||0,tot=t.guidePay+dr+oe;html+='<tr><td style="white-space:nowrap;">'+fmt(t.date)+'</td><td>'+typeBadge(t.type)+'</td><td>'+badge(t.channel)+'</td><td>'+getPax(t)+' pax</td><td class="mono pos">$'+t.guidePay.toFixed(2)+'</td><td class="mono" style="color:var(--amber);">$'+dr.toFixed(2)+'</td><td class="mono" style="color:var(--amber);">'+(oe>0?'$'+oe.toFixed(2):'&mdash;')+'</td><td class="mono" style="font-weight:600;">$'+tot.toFixed(2)+'</td><td style="color:var(--text3);font-size:12px;">'+(t.notes||'&mdash;')+'</td></tr>';});
    html+='</tbody><tfoot><tr><td colspan="4" style="padding:12px;font-weight:600;">Total</td><td class="mono pos" style="padding:12px;font-weight:600;">$'+gPay.toFixed(2)+'</td><td class="mono" style="color:var(--amber);padding:12px;font-weight:600;">$'+gDrinks.toFixed(2)+'</td><td class="mono" style="color:var(--amber);padding:12px;font-weight:600;">$'+gOther.toFixed(2)+'</td><td class="mono" style="font-weight:700;padding:12px;font-size:14px;">$'+gTotal.toFixed(2)+'</td><td></td></tr></tfoot></table></div>';
    html+='<div style="margin-top:1rem;padding:12px 16px;background:var(--bg);border-radius:var(--rs);border:1px solid var(--border);display:flex;gap:24px;flex-wrap:wrap;"><div style="font-size:12px;color:var(--text2);"><strong>For 1099:</strong> Tour pay = <span class="mono" style="color:var(--teal);font-weight:600;">$'+gPay.toFixed(2)+'</span></div><div style="font-size:12px;color:var(--text2);"><strong>Reimbursements</strong> (not taxable) = <span class="mono" style="color:var(--amber);font-weight:600;">$'+gReimb.toFixed(2)+'</span></div></div></div>';
  });
  document.getElementById('pr-content').innerHTML=html;
}

function exportPayCSV() {
  var m=document.getElementById('pr-month').value,sg=document.getElementById('pr-guide').value;
  var data=tours.filter(function(t){return(m==='all'||fmtM(t.date)===parseInt(m))&&(sg==='all'||t.guide===sg);}).sort(function(a,b){return new Date(a.date)-new Date(b.date);});
  var rows=[['Guide','Date','Type','Channel','Guests','Tour Pay','Drinks','Other','Total Reimb','Total Owed','Notes']];
  data.forEach(function(t){var dr=t.drinks||0,oe=t.otherExp||0;rows.push([t.guide,t.date,t.type,ch(t.channel).label,getPax(t),t.guidePay.toFixed(2),dr.toFixed(2),oe.toFixed(2),(dr+oe).toFixed(2),(t.guidePay+dr+oe).toFixed(2),t.notes||'']);});
  downloadCSV(rows,'MWT_PayReport.csv');
}

function openPayModal(id) {
  var t=tours.find(function(t){return t.id===id;});if(!t)return;
  modalTourId=id;
  var vn=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;
  document.getElementById('modal-label').textContent=fmt(t.date)+' — '+ch(t.channel).label+' — Expected: $'+vn.toFixed(2);
  document.getElementById('modal-date').value=new Date().toISOString().split('T')[0];
  document.getElementById('modal-amount').value=vn.toFixed(2);
...
function savePayment() {
  ...
  var t=tours.find(function(t){return t.id===modalTourId;});
  if(t){t.paidDate=date;t.paidAmount=amount;}
  saveTours();if(t)saveTour(t);
  closeModal();renderPayments();renderDashboard();
  toast('Payment recorded!');
}
function unmarkPaid(id) {
  if(!confirm('Remove payment record?'))return;
  var t=tours.find(function(t){return t.id===id;});
  if(t){delete t.paidDate;delete t.paidAmount;}
  saveTours();if(t)saveTour(t);
  renderPayments();renderDashboard();toast('Payment removed.');
}
function logVendorPayment() {
  var channel=document.getElementById('vp-channel').value;
  var date=document.getElementById('vp-date').value;
  var amount=parseFloat(document.getElementById('vp-amount').value)||0;
  var notes=document.getElementById('vp-notes').value;
  if(!date){toast('Please enter a date.',true);return;}
  if(!(amount>0)){toast('Please enter an amount.',true);return;}
  var p={id:Date.now(),channel:channel,date:date,amount:amount,notes:notes};
  vendorPayments.push(p);saveVP();saveVPRecord(p);
  document.getElementById('vp-amount').value='';document.getElementById('vp-notes').value='';
  renderPayments();renderDashboard();toast('Vendor payment saved!');
}
function deleteVP(id) {
  if(!confirm('Delete this vendor payment?'))return;
  vendorPayments=vendorPayments.filter(function(p){return p.id!==id;});
  saveVP();deleteVPDB(id);renderPayments();renderDashboard();toast('Deleted.');
}

function renderPayments() {
  var m=document.getElementById('pay-mf').value;
  var tourData=(m==='all'?tours:tours.filter(function(t){return fmtM(t.date)===parseInt(m);})).sort(function(a,b){return new Date(b.date)-new Date(a.date);});
  var paidC=tourData.filter(function(t){return t.paidDate;}).length;
  var unpaidC=tourData.filter(function(t){return !t.paidDate;}).length;
  document.getElementById('tour-pay-summary').innerHTML='<span style="color:var(--teal);font-weight:600;">'+paidC+' paid</span> &nbsp;&bull;&nbsp; <span style="color:var(--amber);font-weight:600;">'+unpaidC+' pending</span>';
  var totalGross=tourData.reduce(function(s,t){return s+t.gross;},0);
  var totalNet=tourData.reduce(function(s,t){var net=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;return s+net;},0);
  var totalReceived=tourData.filter(function(t){return t.paidDate;}).reduce(function(s,t){return s+(t.paidAmount||0);},0);
  var totalPending=tourData.filter(function(t){return !t.paidDate;}).reduce(function(s,t){var net=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;return s+net;},0);
  if(tourData.length===0){document.getElementById('tour-pay-table').innerHTML='<div class="empty">No tours.</div>';}
  else {
    var html='<table><thead><tr><th>Date</th><th>Tour</th><th>Channel</th><th>Gross</th><th>Expected net</th><th>Status</th><th>Paid date</th><th>Received</th><th></th></tr></thead><tbody>';
    tourData.forEach(function(t){
      var paid=!!t.paidDate;
      var netExp=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;
      var product=t.product||'Miami Wow Tour';
      var pb=product==='Little Havana After Dark'?'<span class="badge b-purple" style="font-size:10px;">LH</span>':'<span class="badge b-teal" style="font-size:10px;">MWT</span>';
      html+='<tr><td style="white-space:nowrap;font-size:12px;">'+fmt(t.date)+'</td><td>'+pb+'</td><td>'+badge(t.channel)+'</td><td class="mono" style="font-size:12px;">$'+t.gross.toFixed(2)+'</td><td class="mono pos" style="font-size:12px;font-weight:600;">$'+netExp.toFixed(2)+'</td><td>'+(paid?'<span class="badge b-green">Paid</span>':'<span class="badge b-yellow">Pending</span>')+'</td><td style="font-size:12px;color:var(--text2);">'+(paid?fmt(t.paidDate):'&mdash;')+'</td><td class="mono" style="font-size:12px;color:'+(paid?'var(--teal)':'var(--text3)')+';">'+(paid?'$'+(t.paidAmount||0).toFixed(2):'&mdash;')+'</td><td>'+(paid?'<button class="btn-danger" onclick="unmarkPaid('+t.id+')">Undo</button>':'<button class="btn-secondary btn-sm" onclick="openPayModal('+t.id+')">Mark paid</button>')+'</td></tr>';
    });
    html+='</tbody><tfoot><tr><td colspan="3" style="padding:10px 12px;font-weight:600;font-size:12px;">Total</td><td class="mono" style="padding:10px 12px;font-size:12px;font-weight:600;">$'+totalGross.toFixed(2)+'</td><td class="mono pos" style="padding:10px 12px;font-size:12px;font-weight:600;">$'+totalNet.toFixed(2)+'</td><td colspan="2" style="padding:10px 12px;font-size:12px;color:var(--text2);">'+paidC+' of '+tourData.length+' paid</td><td class="mono pos" style="padding:10px 12px;font-size:12px;font-weight:600;">$'+totalReceived.toFixed(2)+'</td><td></td></tr><tr><td colspan="3" style="padding:6px 12px;font-size:12px;color:var(--amber);font-weight:600;">Pending (net)</td><td colspan="5" style="padding:6px 12px;font-size:12px;font-weight:600;" class="mono">$'+totalPending.toFixed(2)+'</td><td></td></tr></tfoot></table>';
    document.getElementById('tour-pay-table').innerHTML=html;
  }
  var vpData=(m==='all'?vendorPayments:vendorPayments.filter(function(p){return fmtM(p.date)===parseInt(m);})).sort(function(a,b){return new Date(b.date)-new Date(a.date);});
  if(vpData.length===0){document.getElementById('vendor-pay-table').innerHTML='<div style="font-size:13px;color:var(--text3);padding:8px 0;">No vendor payments logged yet.</div>';}
  else {
    var vhtml='<table><thead><tr><th>Date</th><th>Vendor</th><th>Amount</th><th>Notes</th><th></th></tr></thead><tbody>';
    vpData.forEach(function(p){vhtml+='<tr><td style="white-space:nowrap;font-size:12px;">'+fmt(p.date)+'</td><td>'+badge(p.channel)+'</td><td class="mono pos" style="font-size:12px;">$'+p.amount.toFixed(2)+'</td><td style="font-size:12px;color:var(--text2);">'+(p.notes||'&mdash;')+'</td><td><button class="btn-danger" onclick="deleteVP('+p.id+')">Delete</button></td></tr>';});
    vhtml+='</tbody></table>';
    document.getElementById('vendor-pay-table').innerHTML=vhtml;
  }
}
...
function addExpense() {
  ...
  var e={id:Date.now(),date:date,amount:amount,desc:desc,paidby:paidby};
  expenses.push(e);saveExp();saveExpRecord(e);
  document.getElementById('exp-amount').value='';document.getElementById('exp-desc').value='';
  renderExpenses();toast('Expense added!');
}
function deleteExpense(id) {
  if(!confirm('Delete this expense?'))return;
  expenses=expenses.filter(function(e){return e.id!==id;});
  saveExp();deleteExpDB(id);renderExpenses();toast('Deleted.');
}
function renderExpenses() {
  var m=document.getElementById('exp-mf').value;
  var filtered=m==='all'?expenses:expenses.filter(function(e){return fmtM(e.date)===parseInt(m);});
  var sorted=filtered.slice().sort(function(a,b){return new Date(b.date)-new Date(a.date);});
  var totalAll=expenses.reduce(function(s,e){return s+e.amount;},0);
  var totalFiltered=filtered.reduce(function(s,e){return s+e.amount;},0);
  document.getElementById('exp-total').textContent='$'+totalAll.toFixed(2);
  document.getElementById('exp-period').textContent='$'+totalFiltered.toFixed(2);
  document.getElementById('exp-count').textContent=filtered.length;
  if(sorted.length===0){document.getElementById('expenses-table').innerHTML='<div class="empty">No expenses logged yet.</div>';return;}
  var running=0;
  var withRunning=sorted.slice().reverse().map(function(e){running+=e.amount;return{e:e,r:running};}).reverse();
  var html='<table><thead><tr><th>Date</th><th>Description</th><th>Paid by</th><th style="text-align:right;">Amount</th><th style="text-align:right;">Running total</th><th></th></tr></thead><tbody>';
  withRunning.forEach(function(item){var e=item.e,r=item.r;html+='<tr><td style="white-space:nowrap;font-size:13px;">'+fmt(e.date)+'</td><td style="font-size:13px;">'+e.desc+'</td><td><span class="badge b-gray" style="font-size:11px;">'+e.paidby+'</span></td><td class="mono neg" style="text-align:right;">$'+e.amount.toFixed(2)+'</td><td class="mono" style="text-align:right;color:var(--text2);font-size:12px;">$'+r.toFixed(2)+'</td><td><button class="btn-danger" onclick="deleteExpense('+e.id+')">Delete</button></td></tr>';});
  html+='</tbody><tfoot><tr><td colspan="3" style="padding:12px;font-weight:600;">'+(m==='all'?'Year total':'Period total')+'</td><td class="mono neg" style="text-align:right;padding:12px;font-weight:700;font-size:14px;">$'+totalFiltered.toFixed(2)+'</td><td colspan="2"></td></tr></tfoot></table>';
...
function exportExpCSV() {
  ...
  var rows=[['Date','Description','Paid By','Amount']];
  sorted.forEach(function(e){rows.push([e.date,e.desc,e.paidby,e.amount.toFixed(2)]);});
  rows.push(['','','TOTAL',filtered.reduce(function(s,e){return s+e.amount;},0).toFixed(2)]);
  downloadCSV(rows,'MWT_Expenses_2026.csv');
}
...
function exportCSV() {
  var rows=[['Date','Type','Guide','Adults','Kids','U7','Pax','Channel','Gross','Comm','FH Proc','FH Flat','Guide Pay','Drinks','Other','Profit','Notes']];
  filteredTours().sort(function(a,b){return new Date(a.date)-new Date(b.date);}).forEach(function(t){rows.push([t.date,t.type,t.guide,t.adults,t.children,t.infants,getPax(t),ch(t.channel).label,t.gross.toFixed(2),t.comm.toFixed(2),(t.fhProc||0).toFixed(2),(t.fhFlat||0).toFixed(2),t.guidePay.toFixed(2),(t.drinks||0).toFixed(2),(t.otherExp||0).toFixed(2),t.profit.toFixed(2),t.notes||'']);});
  downloadCSV(rows,'MWT_Tours_2026.csv');
}
function exportBackup() {
  var data={version:1,exported:new Date().toISOString(),tours:tours,vendorPayments:vendorPayments,expenses:expenses};
  var a=document.createElement('a');
  a.href='data:application/json;charset=utf-8,'+encodeURIComponent(JSON.stringify(data,null,2));
  var d=new Date();
  a.download='MWT_Backup_'+d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')+'.json';
...
function importBackup(event) {
  ...
  reader.onload=function(e){
    try{
      ...
      if(!data.tours){toast('Invalid backup file.',true);return;}
      if(!confirm('Replace ALL current data with backup?')){document.getElementById('import-file').value='';return;}
      tours=data.tours||[];vendorPayments=data.vendorPayments||[];expenses=data.expenses||[];
      saveTours();saveVP();saveExp();
      renderDashboard();renderTours();renderGuides();
      toast('Backup imported! '+tours.length+' tours loaded.');
    }catch(err){toast('Error reading file.',true);}
...
function renderReconcile() {
  var m=document.getElementById('rec-month').value;
  var paidTours=tours.filter(function(t){if(!t.paidDate)return false;if(m==='all')return true;return fmtM(t.paidDate)===parseInt(m);}).sort(function(a,b){return new Date(b.paidDate)-new Date(a.paidDate);});
  var totalDeposits=paidTours.reduce(function(s,t){return s+(t.paidAmount||0);},0);
  var totalExpected=paidTours.reduce(function(s,t){return s+(t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100);},0);
  var totalGross=paidTours.reduce(function(s,t){return s+t.gross;},0);
  var diff=Math.round((totalDeposits-totalExpected)*100)/100;
  document.getElementById('rec-summary-cards').innerHTML='<div class="metric"><div class="metric-label">Deposits recorded</div><div class="metric-value">'+paidTours.length+'</div></div><div class="metric"><div class="metric-label">Total gross sales</div><div class="metric-value">$'+totalGross.toFixed(2)+'</div></div><div class="metric"><div class="metric-label">Expected deposits</div><div class="metric-value green">$'+totalExpected.toFixed(2)+'</div></div><div class="metric"><div class="metric-label">Actual received</div><div class="metric-value '+(Math.abs(diff)===0?'green':'amber')+'">$'+totalDeposits.toFixed(2)+'</div></div>';
  if(paidTours.length===0){document.getElementById('rec-vendor-summary').innerHTML='<div class="empty">No paid tours found.</div>';document.getElementById('rec-detail').innerHTML='<div class="empty">No paid tours found.</div>';return;}
  var byVendor={};
  paidTours.forEach(function(t){var v=ch(t.channel).label;if(!byVendor[v])byVendor[v]={tours:0,gross:0,expected:0,received:0};var net=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;byVendor[v].tours++;byVendor[v].gross+=t.gross;byVendor[v].expected+=net;byVendor[v].received+=(t.paidAmount||0);});
  var vsHtml='<table><thead><tr><th>Vendor</th><th>Tours paid</th><th>Gross sales</th><th>Expected deposit</th><th>Actual received</th><th>Difference</th></tr></thead><tbody>';
  var grandGross=0,grandExp=0,grandRec=0;
  Object.keys(byVendor).sort().forEach(function(v){var d=byVendor[v],vdiff=Math.round((d.received-d.expected)*100)/100;grandGross+=d.gross;grandExp+=d.expected;grandRec+=d.received;vsHtml+='<tr><td style="font-weight:600;">'+v+'</td><td>'+d.tours+'</td><td class="mono">$'+d.gross.toFixed(2)+'</td><td class="mono pos">$'+d.expected.toFixed(2)+'</td><td class="mono">$'+d.received.toFixed(2)+'</td><td class="mono '+(Math.round(Math.abs(vdiff)*100)===0?'pos':'neg')+'">'+(vdiff>=0?'+':'')+vdiff.toFixed(2)+'</td></tr>';});
  var grandDiff=Math.round((grandRec-grandExp)*100)/100;
  vsHtml+='</tbody><tfoot><tr><td style="padding:12px;font-weight:600;">Total</td><td style="padding:12px;font-weight:600;">'+paidTours.length+'</td><td class="mono" style="padding:12px;font-weight:600;">$'+grandGross.toFixed(2)+'</td><td class="mono pos" style="padding:12px;font-weight:600;">$'+grandExp.toFixed(2)+'</td><td class="mono" style="padding:12px;font-weight:600;">$'+grandRec.toFixed(2)+'</td><td class="mono '+(Math.round(Math.abs(grandDiff)*100)===0?'pos':'neg')+'" style="padding:12px;font-weight:700;font-size:14px;">'+(grandDiff>=0?'+':'')+grandDiff.toFixed(2)+'</td></tr></tfoot></table>';
  document.getElementById('rec-vendor-summary').innerHTML=vsHtml;
  var detHtml='<table><thead><tr><th>Paid date</th><th>Tour date</th><th>Tour</th><th>Vendor</th><th>Guide</th><th>Guests</th><th>Gross</th><th>Comm</th><th>Expected</th><th>Received</th><th>Diff</th></tr></thead><tbody>';
  paidTours.forEach(function(t){var net=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;var rec=t.paidAmount||0,tdiff=Math.round((rec-net)*100)/100;var product=t.product||'Miami Wow Tour';var pb=product==='Little Havana After Dark'?'<span class="badge b-purple" style="font-size:10px;">LH</span>':'<span class="badge b-teal" style="font-size:10px;">MWT</span>';detHtml+='<tr><td style="white-space:nowrap;font-weight:600;">'+fmt(t.paidDate)+'</td><td style="white-space:nowrap;color:var(--text2);">'+fmt(t.date)+'</td><td>'+pb+'</td><td>'+badge(t.channel)+'</td><td>'+t.guide+'</td><td>'+getPax(t)+'</td><td class="mono">$'+t.gross.toFixed(2)+'</td><td class="mono" style="color:var(--red);">-$'+t.comm.toFixed(2)+'</td><td class="mono pos" style="font-weight:600;">$'+net.toFixed(2)+'</td><td class="mono">$'+rec.toFixed(2)+'</td><td class="mono '+(Math.round(Math.abs(tdiff)*100)===0?'pos':'neg')+'">'+(tdiff>=0?'+':'')+tdiff.toFixed(2)+'</td></tr>';});
  detHtml+='</tbody></table>';
  document.getElementById('rec-detail').innerHTML=detHtml;
}
function exportReconcileCSV() {
  var m=document.getElementById('rec-month').value;
  var paidTours=tours.filter(function(t){if(!t.paidDate)return false;if(m==='all')return true;return fmtM(t.paidDate)===parseInt(m);}).sort(function(a,b){return new Date(a.paidDate)-new Date(b.paidDate);});
  var rows=[['Paid Date','Tour Date','Product','Vendor','Guide','Guests','Gross','Commission','FH Flat','Expected Deposit','Received','Difference']];
  paidTours.forEach(function(t){var net=t.vendorNet>0?t.vendorNet:Math.round((t.gross-t.comm-(t.fhFlat||0))*100)/100;var rec=t.paidAmount||0,tdiff=Math.round((rec-net)*100)/100;rows.push([t.paidDate,t.date,t.product||'Miami Wow Tour',ch(t.channel).label,t.guide,getPax(t),t.gross.toFixed(2),t.comm.toFixed(2),(t.fhFlat||0).toFixed(2),net.toFixed(2),rec.toFixed(2),(tdiff>=0?'+':'')+tdiff.toFixed(2)]);});
  downloadCSV(rows,'MWT_BankReconciliation_2026.csv');
}
...
function openEditModal(id) {
  ...
  editTourId=id;
  var product=t.product||'Miami Wow Tour';
  document.getElementById('em-product').value=product;
  document.getElementById('em-date').value=t.date;
  document.getElementById('em-type').value=t.type||'regular';
  document.getElementById('em-guide').value=t.guide;
  document.getElementById('em-adults').value=t.adults||0;
  document.getElementById('em-children').value=t.children||0;
  document.getElementById('em-infants').value=t.infants||0;
  document.getElementById('em-channel').value=t.channel;
  document.getElementById('em-expenses').value=t.otherExp||0;
  document.getElementById('em-notes').value=t.notes||'';
  var pax=(t.adults||0)+(t.children||0)+(t.infants||0)||getPax(t);
  var autoGP=t.type==='private'?80:(t.guide==='Noel'?(pax>3?100:75):(pax>3?75:60));
  document.getElementById('em-guidepay').value=(t.guidePay!==undefined&&Math.abs(t.guidePay-autoGP)>0.01)?t.guidePay.toFixed(2):'';
  var tour=TOURS[product]||TOURS['Miami Wow Tour'];
  document.getElementById('em-adults-label').textContent='Adults ($'+tour.adultPrice+')';
  document.getElementById('em-children-label').textContent='Children ($'+tour.childPrice+')';
  document.getElementById('edit-modal-bg').style.display='flex';
  setTimeout(function(){previewEdit();},50);
}
function previewEdit() {
  var product=document.getElementById('em-product').value;
  var adults=parseInt(document.getElementById('em-adults').value,10)||0;
  var children=parseInt(document.getElementById('em-children').value,10)||0;
  var infants=parseInt(document.getElementById('em-infants').value,10)||0;
  var guide=document.getElementById('em-guide').value;
  var channel=document.getElementById('em-channel').value;
  var type=document.getElementById('em-type').value;
  var exp=parseFloat(document.getElementById('em-expenses').value)||0;
  var tour=TOURS[product]||TOURS['Miami Wow Tour'];
  document.getElementById('em-adults-label').textContent='Adults ($'+tour.adultPrice+')';
  document.getElementById('em-children-label').textContent='Children ($'+tour.childPrice+')';
  var d=calcTourData(adults,children,infants,guide,channel,type,exp,product);
  document.getElementById('em-gross').textContent='$'+d.gross.toFixed(2);
  document.getElementById('em-comm').textContent='-$'+d.comm.toFixed(2)+' ('+Math.round(CHANNELS[channel].rate*100)+'%)';
  var fhflatRow=document.getElementById('em-fhflat-row');if(fhflatRow)fhflatRow.style.display=channel==='Fareharbor'?'flex':'none';
  var fhprocRow=document.getElementById('em-fhproc-row');if(fhprocRow)fhprocRow.style.display='flex';
  document.getElementById('em-fhproc').textContent='-$'+d.fhProc.toFixed(2);
  var gpOverride=parseFloat(document.getElementById('em-guidepay').value);
  var gpDisplay=(!isNaN(gpOverride)&&gpOverride>=0)?gpOverride:d.guidePay;
  document.getElementById('em-guide-pay').textContent='-$'+gpDisplay.toFixed(2)+((!isNaN(gpOverride)&&gpOverride>=0)?' (override)':' (auto)');
  var drinksRow=document.getElementById('em-drinks-row');if(drinksRow)drinksRow.style.display=product==='Miami Wow Tour'?'flex':'none';
  document.getElementById('em-drinks').textContent='-$'+d.drinks.toFixed(2);
  document.getElementById('em-exp').textContent=exp>0?'-$'+exp.toFixed(2):'$0.00';
  var pp=Math.round((d.gross-d.comm-(d.fhFlat||0)-(d.fhProc||0)-gpDisplay-(d.drinks||0)-exp)*100)/100;
  var pel=document.getElementById('em-profit');
  pel.textContent=(pp>=0?'$':'-$')+Math.abs(pp).toFixed(2);
  pel.style.color=pp>=0?'var(--teal)':'var(--red)';
}
function saveEdit() {
  if(!editTourId)return;
  var t=tours.find(function(t){return t.id===editTourId;});if(!t)return;
  var product=document.getElementById('em-product').value;
  var adults=parseInt(document.getElementById('em-adults').value,10)||0;
  var children=parseInt(document.getElementById('em-children').value,10)||0;
  var infants=parseInt(document.getElementById('em-infants').value,10)||0;
  ...
  var d=calcTourData(adults,children,infants,guide,channel,type,exp,product);
  var gpOverride=parseFloat(document.getElementById('em-guidepay').value);
  var finalGuidePay=(!isNaN(gpOverride)&&gpOverride>=0)?Math.round(gpOverride*100)/100:d.guidePay;
  var finalProfit=Math.round((d.gross-d.comm-(d.fhFlat||0)-(d.fhProc||0)-finalGuidePay-(d.drinks||0)-exp)*100)/100;
  t.product=product;t.date=date;t.type=type;t.guide=guide;
  t.adults=adults;t.children=children;t.infants=infants;t.pax=adults+children+infants;
  t.channel=channel;t.otherExp=exp;t.notes=notes;
  t.gross=d.gross;t.comm=d.comm;t.fhFlat=d.fhFlat;t.fhProc=d.fhProc;
  t.vendorNet=Math.round((d.gross-d.comm-(d.fhFlat||0))*100)/100;
  t.guidePay=finalGuidePay;t.drinks=d.drinks;t.profit=finalProfit;
  saveTours();saveTour(t);
  closeEditModal();renderTours();renderDashboard();renderGuides();
  toast('Tour updated!');
}
